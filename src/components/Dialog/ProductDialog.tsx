...
const ProductDialog: React.FC<ProductDialogProps> = ({ dialogData, handleClose }) => {
  ...
  const { mutate: addProduct } = AddProduct.useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["getProductList"] }); // Refresh product list
      toast({
  ...
  useEffect(() => {
    if (dialogData.isEdit && dialogData.data) {
      reset({
        ...
        profitPerItem: dialogData.data.profitPerItem || 0,
      });
    } else {
      reset({
        productName: "",
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid gap-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="profitPerItem" className="text-right">Profit Per Item</Label>
                <Input id="profitPerItem" type="number" {...register("profitPerItem", { valueAsNumber: true })} className="col-span-3" />
              </div>
            </div>
            <DialogFooter>
